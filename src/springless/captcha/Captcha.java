...
public class Captcha {
	...
	public static String generateText() {
		return new StringTokenizer(UUID.randomUUID().toString(), "-")
				.nextToken();
	}
	 ...
	 *
	 * @param text
	 *            expects string size eight (8) characters.
	...
	public static byte[] generateImage(String text) {
		...
		g.setColor(Color.white);
		for (int i = 0; i < 8; i++) {
			g.drawOval((int) (Math.random() * 160), (int) (Math.random() * 10),
					30, 30);
		}
		g.dispose();
